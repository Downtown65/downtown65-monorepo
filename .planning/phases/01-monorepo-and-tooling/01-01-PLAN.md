...
must_haves:
  truths:
    - "Running `pnpm install` from repo root installs all workspace dependencies without errors"
    - "Running `pnpm biome check .` from repo root lints and formats all packages"
    - "Running `pnpm knip` from repo root detects unused code across all workspaces without false positives"
    - "Running `pnpm dlx sherif@latest` passes with no dependency version inconsistencies"
    - "Running `pnpm tsc --build` typechecks all workspaces without errors"
...
<task type="auto">
  <name>Task 1: Create monorepo workspace structure and TypeScript configuration</name>
  <files>
    pnpm-workspace.yaml
    package.json
    .npmrc
  ...
  <action>
    Create the full monorepo directory structure from scratch. This is a green-field project — no existing code.
    ...
    **App scaffolds (apps/api, apps/events, apps/www):**
    ...
    10. `src/index.ts` — Minimal placeholder for each app:
    ...
    **Shared package (packages/shared):**
    ...
    13. `src/index.ts` — Barrel export placeholder:
        ```typescript
...
<task type="auto">
  ...
  <action>
    ...
    **3. `lefthook.yml` — Pre-push hooks:**
    ...
    Note: `--noEmit` with `--build` ensures typecheck without writing output files. The hooks run in parallel for speed.
