  ...
  <action>
    ...
    **Job 2: `preview-deploy`** — Deploy preview Workers
    ...
    - Steps:
      ...
      3. `actions/setup-node@v4` with `node-version: 22`, `cache: pnpm`
      4. `pnpm install --frozen-lockfile`
      5. **Create per-PR D1 database and capture its ID:**
         ```bash
         ...
         ```
         Note: `wrangler d1 list --json` does not exist in wrangler v4. Instead, capture the UUID from the `wrangler d1 create` output directly (it prints the database ID on creation), and fall back to `wrangler d1 info $DB_NAME --json` for re-runs where the DB already exists.
    ...
    **Important notes:**
    ...
    - Both `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` are set as job-level env vars from secrets. This makes them available to both direct `npx wrangler` commands (step 5) and `cloudflare/wrangler-action@v3` steps (steps 6-8). The wrangler-action reads these env vars automatically.
    ...
    **Job: `cleanup`**
    ...
    - Steps:
      ...
      6. **Delete per-PR D1 database:**
         ...
         ```
         Note: `wrangler d1 list --json` does not exist in wrangler v4. Use `wrangler d1 info $DB_NAME --json` to retrieve the database UUID before deletion.
